...
for i,nodeTag in enumerate(nodeTagList):
    ...
    autovector = []
    for eigenMode in range(1, numEigenvalues + 1):
            NodeModalDisp = ops.nodeEigenvector( nodeTag ,eigenMode, 1) # spostamenti e rotazioni del nodo 
            
            autovector.append( NodeModalDisp )
    autovectorX.append( autovector )

coefPartX = []
massPartX = []

for eigenMode in range(0, numEigenvalues ) :
    autovector = [row[eigenMode] for row in autovectorX ] 
    passo1 = multiply(autovector, massXMatrix)
    c_den = sum( i*j for i,j in zip( autovector, passo1)) # coefficiente al denominatore
    passo2 = multiply(R, massXMatrix)
    c_num = sum( i*j for i,j in zip( autovector, passo2)) # coefficiente al numeratore
    if c_den == 0 :
        ...
        # -- coefficiente di partecipazione -- #
        coefPartX.append( c_num/c_den ) 
        # -- coefficiente di partecipazione -- #
        massPartX.append( (c_num/c_den)**2*c_den ) 
...
for i,nodeTag in enumerate(nodeTagList):
    listMassMatrix = zerolistmaker( lenMassMatrix )
    listMassMatrix[i] = ops.nodeMass( nodeTag , 2)
    massYMatrix.append( listMassMatrix )
    autovector = []
    for eigenMode in range(1, numEigenvalues + 1):
            NodeModalDisp = ops.nodeEigenvector( nodeTag ,eigenMode, 2) # spostamenti e rotazioni del nodo 
            autovector.append( NodeModalDisp )
    autovectorY.append( autovector )

coefPartY = []
massPartY = []

for eigenMode in range(0, numEigenvalues ) :
    autovector = [row[eigenMode] for row in autovectorY ] 
    passo1 = multiply(autovector, massYMatrix)
    c_den = sum( i*j for i,j in zip( autovector, passo1)) # coefficiente al denominatore
    passo2 = multiply(R, massYMatrix)
    c_num = sum( i*j for i,j in zip( autovector, passo2)) # coefficiente al numeratore
    if c_den == 0 :
        # -- coefficiente di partecipazione -- #
        coefPartY.append( 0 ) 
        ...
        # -- coefficiente di partecipazione -- #
        coefPartY.append( c_num/c_den ) 
        # -- coefficiente di partecipazione -- #
        massPartY.append( (c_num/c_den)**2*c_den ) 

MtotY = sum( massPartY )
#--------------------------#

percMassPart = []
for i,j in zip(massPartX,massPartY) :
    percMassPart.append( [i/ MtotX, j/ MtotY ]  )

cPart = []
for i,j in zip(coefPartX,coefPartY) :
    cPart.append( [i, j ]  )
