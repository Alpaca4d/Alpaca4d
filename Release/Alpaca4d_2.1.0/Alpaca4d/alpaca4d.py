...
class ForceBeamColumn(object):
    ...
    def __init__(self, Crv, CrossSection, GeomTransfType, Colour, Z_Axis):
...
class doubleTCS(object):
    def __init__(self, Bsup, tsup, Binf, tinf, H, tw, material = None):
        ...
        p1 = plane.PointAt( -(self.yg - self.tinf), self.tw/2 )
        p2 = plane.PointAt( -(self.yg - self.tinf), self.Binf/2 )
        p3 = plane.PointAt( -self.yg, self.Binf/2 )
        p4 = plane.PointAt( -self.yg, -self.Binf/2 )
        p5 = plane.PointAt( -(self.yg - self.tinf), -self.Binf/2 ) 
        p6 = plane.PointAt( -(self.yg - self.tinf), -self.tw/2 )
        p7 = plane.PointAt( (self.H - self.yg - self.tsup), -self.tw/2)
        p8 = plane.PointAt( (self.H - self.yg - self.tsup), -self.Bsup/2 )
        p9 = plane.PointAt( (self.H - self.yg ), -self.Bsup/2 )
        p10 = plane.PointAt( (self.H - self.yg ), self.Bsup/2 )
        p11 = plane.PointAt( (self.H - self.yg - self.tsup), self.Bsup/2 )
        p12 = plane.PointAt( (self.H - self.yg - self.tsup), self.tw/2 ) 
...
class Analysis(object):
    def __init__(self, Constraint, Numberer, System, Test, Algorithm, Integrator, AnalysisType, AnalysisSteps):
