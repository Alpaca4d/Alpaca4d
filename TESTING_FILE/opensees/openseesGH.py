...
class Model(object):
    ...
    def getuniquePoints(self, beams, shells, bricks):
...
class ElasticOrthotropic(object):
    def __init__(self, matName, Ex, Ey, Ez, vxy, vyz, vzx, Gxy, Gyz, Gzx, rho=0.0, fy=None):
...
class ForceBeamColumn(object):
    ...
    def __init__(self, Crv, CrossSection, Color, OrientSection):
...
class ShellMITC4(object):
    def __init__(self, Mesh, CrossSection, Color):
...
class ASDShellQ4(object):
    ...
    def write_tcl(self):
        # https://opensees.berkeley.edu/wiki/index.php/Shell_Element
        # element ShellMITC4 $eleTag $iNode $jNode $kNode $lNode $secTag
        return "element ASDShellQ4 {} {} {} {} {} {}\n".format(self.eleTag, self.indexNodes[0], self.indexNodes[1], self.indexNodes[2], self.indexNodes[3], self.CrossSection.sectionTag)

    def write_py(self):
        return "ops.element('ASDShellQ4', {}, {}, {}, {}, {}, {})\n".format(self.eleTag, self.indexNodes[0], self.indexNodes[1], self.indexNodes[2], self.indexNodes[3], self.CrossSection.sectionTag)

#TODO
class StdBrick(object):
    def __init__(self, Brick, material):
        ...
        self.eleTag = None
        self.indexNodes = None

        self.elementType = "stdBrick"
        #self.elementType = 'bbarBrick'
        
            
        self.Mesh.VertexColors.CreateMonotoneMesh( self.colour )
...
class EqualDOF(object):
    def __init__(self, masterNode, slaveNodes, dof):
        self.masterNode = masterNode
        self.slaveNodes = slaveNodes
        self.dof = dof
...
class PointLoad(object):
    ...
    @staticmethod
    def write_tcl(groupsTimeSeries):
...
class TCLRecorder(object):
    ...
    @staticmethod
    def mpco(fileName, displacement, rotation, reactionForce, reactionMoment, modesOfVibration, modesOfVibrationRotational):
        ...
        if nodeRespType:
            return "recorder mpco {} -N {}".format(fileName, nodeRespType)

def setNodeDisplacement(modelObj, timeStep, dispFilename):

    timeValueDisp = []

    with open(dispFilename, 'r') as f:
        lines = f.readlines()
        for line in lines:
            timeValueDisp.append( line.strip().split(" ")[1:] )

    timeValueDisp = [[float(s) for s in ValueDisp] for ValueDisp in timeValueDisp]
    ...
    for item in timeValueDisp:
        a = chunkify(item,6)
        newTimeValueDisp.append(a)


    dispDict = defaultdict(dict)

    for timeSteps, dispValues in enumerate(newTimeValueDisp,1):
        for id, disp in enumerate(dispValues,1):
            vectorDict = defaultdict(dict)
            vectorDict["x"] = disp[0]
            vectorDict["y"] = disp[1]
            vectorDict["z"] = disp[2]
            vectorDict["xx"] = disp[3]
            vectorDict["yy"] = disp[4]
            vectorDict["zz"] = disp[5]
            dispDict[timeSteps][id] = vectorDict

    for node in modelObj.nodes:
        node.displacement = rg.Vector3d( dispDict[timeStep][node.nodeTag]["x"], dispDict[timeStep][node.nodeTag]["y"], dispDict[timeStep][node.nodeTag]["z"] )
        node.rotation = rg.Vector3d( dispDict[timeStep][node.nodeTag]["xx"], dispDict[timeStep][node.nodeTag]["yy"], dispDict[timeStep][node.nodeTag]["zz"] )
